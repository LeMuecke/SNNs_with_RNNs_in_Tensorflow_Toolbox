...
class IntegratorNeuronCell(tf.keras.layers.Layer):
    ...
    def call(self, input_at_t, states_at_t):
        ...
        input_z, input_v = tf.nest.flatten(input_at_t)

        i_t = tf.add(tf.matmul(input_z, self.w_in), self.b_in)
...
class LifNeuronCell(IntegratorNeuronCell):
    """
    A LifNeuron that uses time-to-first-spike (ttfs) encoding with dynamic threshold.
    ...
    To enable the dynamic threshold, the cell is not outputting only the spikes but also the potential of the cell.
    """
    ...
    def call(self, input_at_t, states_at_t):
        ...
        heaviside_v = tf.cast(tf.math.greater_equal(input_v, 0), tf.float32)
        thresh = self.threshold + tf.matmul(heaviside_v, tf.abs(self.w_in))
        ...
        new_z = tf.maximum(self.activation_function(new_v / thresh), old_z)

        output_at_t = (new_z, new_v)

        return output_at_t, (new_v, new_z)


class LifNeuronCellConv2D(IntegratorNeuronCell):
    # TODO Convolutions seem to be instable, when testing the first 100ish of images I typically get accuracies between 0.85 and 0.97
    # TODO Update: Seems like bigger networks are instable,  not conv itself. More details in thesis.
    """
    A LifNeuron that uses time-to-first-spike (ttfs) encoding with dynamic threshold.
    Be aware that this implementation does not resemble the way ttfs normally works.
    Normally, ttfs would output one spike only, in this implementation, ttfs outputs spikes at every timestep after
    ...
    spiking after the first spike.
    To enable the dynamic threshold, the cell is not outputting only the spikes but also the potential of the cell.
    This implements ttfs dyn tresh with convolutions.
    """

    def __init__(self, input_shape: int, output_shape: int, tau: float = 999999., threshold: float = 0.1,
                 activation_function: Callable[[tf.Tensor], tuple] = spike_function, kernel_size: (int, int) = (3, 3),
                 filters: int = 3, strides: (int, int) = (1, 1), padding: str = "VALID", data_format: str = "NHWC",
                 dilations: int or list = 1, **kwargs):
        """
        Initializes a (Recurrent)LifNeuronCell.
        @param n_in: Number of inputs, i.e. outputs of previous layer.
        @param n_neurons: Number of neurons, i.e. outputs of this layer.
    ...
    def call(self, input_at_t, states_at_t):
        ...
        # membrane potential that will be added this timestep
        i_t = tf.nn.conv2d(input_z, filters=self.w_in, strides=self.strides, padding=self.padding,
                           data_format=self.data_format, dilations=self.dilations)
        i_t = tf.nn.bias_add(i_t, self.b_in)

        heaviside_v = tf.nn.conv2d(tf.cast(tf.math.greater_equal(input_v, 0), tf.float32), filters=self.w_in, strides=self.strides, padding=self.padding,
                           data_format=self.data_format, dilations=self.dilations)
        # heaviside_v = tf.nn.bias_add(heaviside_v, self.b_in)

        # add new potential of this timestep to the old potential
        new_v = self.alpha * old_v + i_t
        # calculate new threshold
        thresh = self.threshold + heaviside_v
        # new_z -> output_at_t, tf.maximum makes output stay at 1 when the first spike was emitted
        # this is due to the way the Rueckauer TTFS math is translated to TensorFlow (TODO more to come)
        new_z = tf.maximum(self.activation_function(new_v / thresh), old_z)
...
class LifNeuronCellMaxPool2D(IntegratorNeuronCell):
    ...
    spiking after the first spike.
    To enable the dynamic threshold, the cell is not outputting only the spikes but also the potential of the cell.
    This implements ttfs dyn thresh with max pooling.
    """
    ...
    def call(self, input_at_t, states_at_t):
        ...
        pooled_z = tf.nn.max_pool2d(input_z,
                                        ksize=self.pool_size,
                                        strides=self.strides,
                                        padding=self.padding.upper(),
                                        data_format=self.data_format)

        pooled_v = tf.nn.max_pool2d(input_v,
                                        ksize=self.pool_size,
                                        strides=self.strides,
                                        padding=self.padding.upper(),
                                        data_format=self.data_format)

        output_at_t = (pooled_z, pooled_v)

        return output_at_t, (empty,)
